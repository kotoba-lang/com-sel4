(ns sel4.main
  "Kotodama WASM entrypoint for the Sel4 clean-room actor (L4) — Clojure port.
  ...
  facts (`sel4.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"sel4.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sel4-compat" :tier tier :entities entities} 200])
